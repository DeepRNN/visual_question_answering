This is an implementation of the neural network architecture for visual question answering as described by the paper "Dynamic Memory Networks for Visual and Textual Question Answering" by Xiong et al. (ICML2016). The input is an image and a question about the image in natural language, and the output is a single-word answer in natural language. It first uses a convolutional neural network to extract a set of features of the image, which are called "contexts" or "facts". These facts are fused by a bidirectional 
