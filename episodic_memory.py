...
class AttnGRU:
    ...
    def __call__(self, inputs, state, attention):
        with tf.variable_scope('AttnGRU'):
            r = fully_connected(tf.concat([inputs, state], 1), self.num_units, 'AttnGRU_fc1', init_b=1.0)
            r = batch_norm(r, 'AttnGRU_bn1', self.is_train, self.bn)
            r = nonlinear(r, 'sigmoid')

            c = fully_connected(tf.concat([inputs, r*state], 1), self.num_units, 'AttnGRU_fc2', init_b=0.0)
            c = batch_norm(c, 'AttnGRU_bn2', self.is_train, self.bn)
            c = nonlinear(c, 'tanh') 
...
class EpisodicMemory:
    ...
    def attend(self, memory):
        ...
        z1 = fully_connected(z, self.num_units, 'EM_att_fc1')  
        z1 = batch_norm(z1, 'EM_att_bn1', self.is_train, self.bn)
        z1 = nonlinear(z1, 'tanh')

        z2 = fully_connected(z1, 1, 'EM_att_fc2')
        z2 = batch_norm(z2, 'EM_att_bn2', self.is_train, self.bn)                
